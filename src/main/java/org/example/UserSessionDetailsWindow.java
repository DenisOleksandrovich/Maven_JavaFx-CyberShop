...
public class UserSessionDetailsWindow extends Application {
    ...
    public UserSessionDetailsWindow(int clientId) {
    ...
    private void loadSessions() {
        try (Connection connection = establishDBConnection()) {
            ...
            String currentSessionsSql = "SELECT session_id, country, ip_address, device_type, start_time FROM sessions WHERE user_id = ? AND end_time IS NULL";
            PreparedStatement currentSessionsStatement = connection.prepareStatement(currentSessionsSql);
            currentSessionsStatement.setInt(1, clientId);
            ...
            String allSessionsSql = "SELECT session_id, country, ip_address, device_type, start_time, end_time FROM sessions WHERE user_id = ?";
            PreparedStatement allSessionsStatement = connection.prepareStatement(allSessionsSql);
            allSessionsStatement.setInt(1, clientId);
            ...
            while (allSessionsResultSet.next()) {
                ...
                VBox sessionBox = createSessionBox("Session " + sessionNumber, country, ipAddress, deviceType, startTime + " - " + endTime, sessionId);
